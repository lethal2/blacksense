...
do --ANCHOR Visuals
	...
	do --esp and chams
		...
		function visuals.applyEsp(player)
			...
			function this.renderGeneral(info)
				...
				if section:find("Enemy") and Menu["ESP"]["Enemy ESP"]["Show Extrapolation Position"]["Toggle"]["Enabled"] and Menu["Rage"]["Hack vs. Hack"]["Extrapolation"]["Toggle"]["Enabled"] and player.Character and player.Character:FindFirstChild("Humanoid") and this.timePassed == 0 and info.pInfo.velocity.Magnitude > 0.01 then
					...
					local extrapolateby = info.pInfo.velocity * ((Menu["Rage"]["Hack vs. Hack"]["Extrapolation Tuning"]["Value"] + (game.Stats.PerformanceStats.Ping:GetValue() * 2)) / 1000) -- this is how far ahead in time we are extrapolating
	...
	Menu["Visuals"]["Camera"]["Override FOV"]["Toggle"].Changed:Connect(visuals.updateFov)
	Menu["Visuals"]["Camera"]["FOV"].Changed:Connect(visuals.updateFov)

	camera:GetPropertyChangedSignal("FieldOfView"):Connect(function()
		if Menu["Visuals"]["Camera"]["Disable Scope FOV"]["Toggle"]["Enabled"] and Menu["Visuals"]["Camera"]["Override FOV"]["Toggle"]["Enabled"] and localPlayer.Character and localPlayer.Character:FindFirstChild("Humanoid") then 
			camera.FieldOfView = Menu["Visuals"]["Camera"]["FOV"]["Value"] --inb4 maximum re entrance error -- never happened
		end
	end)
...
do --ANCHOR Ragebot
	...
	function ragebot.setupbacktrack(player)
        ...
        this.updateBacktrack = function(upTime, deltaTime)
			...
			local backtrackTime = (Menu["Rage"]["Hack vs. Hack"]["Back Tracking"]["Toggle"]["Enabled"] and (Menu["Rage"]["Hack vs. Hack"]["Back Tracking Time"]["Value"] / 1000) or 0)
	...
	function ragebot.setupfakehrp(char)
		...
		-- this part of code doesnt make you move or something, no idea why?
    ...
    local fakeReplicationUpdate = function(uptime, delta)
		...
		if tick() - ragebot.lastshot < 1/16 and ragebot.currenttarget.position then
		...
		if fireAnimation and client.fgun ~= "none" then
			...
			if localPlayer.Character:FindFirstChild("Gun") then
				...
				if findFirstChild(localPlayer.Character.Gun, "Flash") and findFirstChild(camera, "Arms") and findFirstChild(camera.Arms, "Flash") then
					...
					replicatedStorage.Events.RemoteEvent:FireServer({"createparticle", "muzzle", localPlayer.Character.Gun.Flash, nil})
	...
	function ragebot.fire(Parameters) -- everything that takes place once someone CAN be shot
		...
		if Menu["Rage"]["Aimbot"]["Auto Shoot"]["Toggle"]["Enabled"] then
			for i = 1, client.fgun.Bullets.Value do
				hitPart:FireServer(
					Menu["Rage"]["Hack vs. Hack"]["Force Headshots"]["Toggle"]["Enabled"] and Parameters.instance.Parent.Head or Parameters.instance, -- 1
			...
			print("Shot at", ragebot.currenttarget.player, "for", tostring(math.floor(Parameters.damageInflicted + 0.5)) .. ", in the", ragebot.currenttarget.instance, "(" .. tostring(math.clamp(math.floor(Parameters.instance.Parent.Humanoid.Health - (Parameters.damageInflicted * client.fgun.Bullets.Value) + 0.5), 0, 100)) .. " hp remaining)", "(type:" .. Parameters.type .. ", origin:" .. Parameters.origintype .. ")")
		end
	end
	...
	function ragebot.getorigins(toward)
		...
		local factor = 10
		if Menu["Rage"]["Hack vs. Hack"]["Autowall Hitscan"]["Toggle"]["Enabled"] then
			local offsetOrigins = {}
			if table.find(Menu["Rage"]["Hack vs. Hack"]["Hitscan Points"]["Value"], "Up") ~= nil then
				offsetOrigins[1 + #offsetOrigins] = {(base * newCframe(0, factor, 0)).p, "Up"} -- up
			end
			if table.find(Menu["Rage"]["Hack vs. Hack"]["Hitscan Points"]["Value"], "Down") ~= nil then
				offsetOrigins[1 + #offsetOrigins] = {(base * newCframe(0, -factor, 0)).p, "Down"} -- down
			end
			if table.find(Menu["Rage"]["Hack vs. Hack"]["Hitscan Points"]["Value"], "Left") ~= nil then
				offsetOrigins[1 + #offsetOrigins] = {(base * newCframe(-factor, 0, 0)).p, "Left"} -- left
			end
			if table.find(Menu["Rage"]["Hack vs. Hack"]["Hitscan Points"]["Value"], "Right") ~= nil then
				offsetOrigins[1 + #offsetOrigins] = {(base * newCframe(factor, 0, 0)).p, "Right"} -- right
			end
			if table.find(Menu["Rage"]["Hack vs. Hack"]["Hitscan Points"]["Value"], "Backward") ~= nil then
				offsetOrigins[1 + #offsetOrigins] = {(base * newCframe(0, 0, factor)).p, "Backward"} -- backwards
			end
			if table.find(Menu["Rage"]["Hack vs. Hack"]["Hitscan Points"]["Value"], "Forward") ~= nil then
				offsetOrigins[1 + #offsetOrigins] = {(base * newCframe(0, 0, -factor)).p, "Forward"} -- forwards
			end
			if table.find(Menu["Rage"]["Hack vs. Hack"]["Hitscan Points"]["Value"], "Towards") ~= nil then
		...
		if Menu["Rage"]["Hack vs. Hack"]["Auto Peek"]["Toggle"]["Enabled"] and Menu["Rage"]["Hack vs. Hack"]["Auto Peek"]["Bind"]["Active"] then
	...
	function ragebot.getpoints(enemy)
		...
		-- camera$resolve$
		if Menu["Rage"]["Hack vs. Hack"]["Resolve Positions"]["Toggle"]["Enabled"] then
		...
		if Menu["Rage"]["Hack vs. Hack"]["Extrapolation"]["Toggle"]["Enabled"] and pInfo.velocity.Magnitude > 0.01 then -- since they are moving we should try to predict where they are
			local extrapolatedparam = RaycastParams.new()
			extrapolatedparam.IgnoreWater = true
			extrapolatedparam.FilterType = Enum.RaycastFilterType.Whitelist
			extrapolatedparam.FilterDescendantsInstances = {workspace.Map} -- this cannot be moved through under normal circumstances	

			local extrapolateby = pInfo.velocity * ((Menu["Rage"]["Hack vs. Hack"]["Extrapolation Tuning"]["Value"] + (game.Stats.PerformanceStats.Ping:GetValue() * 2)) / 1000) -- this is how far ahead in time we are extrapolating
			local extrapolatedpoints = ragebot.hitgrouptohitbox(Menu["Rage"]["Hack vs. Hack"]["Extrapolation Points"]["Value"]) -- what hitboxes do the hitgroups in the dropdown allow
			...
			for i, v in next, extrapolatedpoints do
				...
				if correspondinginstance then 
					...
					local extrapolated = cast and cast.Position or center + extrapolateby
					scan[1 + #scan] = {instance, extrapolated, "Extrapolated"} -- record where their extrapolated position is
		...
		if Menu["Rage"]["Hack vs. Hack"]["Back Tracking"]["Toggle"]["Enabled"] then
			local btpoints = ragebot.hitgrouptohitbox(Menu["Rage"]["Hack vs. Hack"]["Back Tracking Points"]["Value"])
		...
		if Menu["Rage"]["Hack vs. Hack"]["Multi Point"]["Toggle"]["Enabled"] then
			local maxPullFactor = Menu["Rage"]["Hack vs. Hack"]["Multi Point Scale"]["Value"] / 100
			local instensity = 4
			local multiPointScan = {}
			local mppoints = ragebot.hitgrouptohitbox(Menu["Rage"]["Hack vs. Hack"]["Multi Point Points"]["Value"]) -- what hitboxes do the hitgroups in the dropdown allow
	...
	function ragebot.scan(enemy) -- calculates the highest damage point on a single target
		...
		do
			...
			local pointsPerGroup = math.floor(Menu["Rage"]["Hack vs. Hack"]["Maximum Hitscanning Points"]["Value"] / groups)
	...
	function ragebot.think() -- Invaded#5143 (girl)
		...
		if workspace.Status.Preparation.Value and Menu["Rage"]["Hack vs. Hack"]["Wait For Round Start"]["Toggle"]["Enabled"] then
				...
				if Menu["Visuals"]["Camera"]["Hit Marker Type"].Value == "2D" then
					...
					task.spawn(function()
						...
						local text = visuals.createDrawing("Text", {
							Color = data[1].Name:match("Head") and Color3.new(1, 0, 0) or Color3.new(1, 1, 1),
							Visible = true,
						...
						if pInfo ~= nil then
							...
							if pInfo.alive and pInfo.enemy then -- ok they are still alive
								...
								if possiblepoints[hit] == true then -- ok so its still a valid point
									...
									if not client.DISABLED and mayProceed == true then
										client.firebullet()
	...
	local oldNamecall; oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
		...
		if method == "FireServer" then
			...
			elseif self.Name == hitPart.Name and not Menu["Misc"]["Extra"]["Kill All"]["Toggle"]["Enabled"] then
				...
				if Menu["Rage"]["Hack vs. Hack"]["Back Tracking"]["Toggle"]["Enabled"] then -- ok its a backtrack part
					local v = {
						real = ragebot.allPartsForBacktrack[args[1]]
					}
					if ragebot.allPartsForBacktrack[args[1]] then
						args[1] = v.real
						args[2] = Vector3.new(((v.real.Position.x - 74312) * 4 + 1325) * 13, (v.real.Position.y + 3183421) * 4 - 4201432, (v.real.Position.z * 41 - 581357) * 2)
					end    
				end
				...
				if args[14] == "r" then
					if Menu["Rage"]["Hack vs. Hack"]["Prediction"]["Toggle"]["Enabled"] then
						if Menu["Rage"]["Hack vs. Hack"]["Prediction Type"]["Value"] == "Exploit" then
							args[2] = {X = math.sqrt(-1), Y = math.sqrt(-1), Z = math.sqrt(-1)}
						else
							local pInfo = playerInfo.storage[dude]
							if pInfo and pInfo.velocity.Magnitude > 1 and #pInfo.updates > 2 then
								local configPing = Menu["Rage"]["Hack vs. Hack"]["Multiplier"]["Value"]
								local simPing = configPing > 0 and configPing or localPing
								local simVel = pInfo.velocity
								args[2] = pInfo.updates[1].position + (simVel * simPing / 1000) + (simVel * 1/60)
								...
								local previousPos = previous.position
								...
								wall.CFrame = CFrame.lookAt(currentPos, currentPos + vel)
								wall.Parent = workspace.Ray_Ignore
								wall.Size = Vector3.new(1, 1, 1)
								wall.Anchored = true
                                
								local currentToLastVec = currentPos - previousPos
								local rayhit, raypos, raynormal = workspace.FindPartOnRayWithWhitelist(workspace, Ray.new(previousPos, currentToLastVec), {wall})

								print("current to last vec", currentToLastVec)
								
								local reflectedVector = trajectory.reflect(vel, -raynormal)

								local simVel = reflectedVector.unit * pInfo.velocity.Magnitude
								
								print("guess vel", simVel)
								print("real vel", pInfo.velocity)

								args[2] = currentPos + (simVel * simPing / 1000) + (simVel * 1/60)
                                ...
                                do
                                    ...
                                    local ind = Instance.new("Part", workspace.Ray_Ignore)
                                    ind.Anchored = true
                                    ind.CanCollide = false
                                    ind.Size = newVector3(0.01, 0.01, (thisframe - prevframe).Magnitude)
                                ...
                                do
                                    local thisframe = currentPos
                                    local prevframe = currentPos + (pInfo.velocity * simPing / 1000) + (pInfo.velocity * 1/60)
                                    local ind = Instance.new("Part", workspace.Ray_Ignore)
                                    ind.Anchored = true
                                    ind.CanCollide = false
                                    ind.Size = newVector3(0.01, 0.01, (thisframe - prevframe).Magnitude)
                            ...
                            args[2] = Vector3.new(((args[2].x - 74312) * 4 + 1325) * 13, (args[2].y + 3183421) * 4 - 4201432, (args[2].z * 41 - 581357) * 2)
						end
					else
						args[2] = Vector3.new(((args[2].x - 74312) * 4 + 1325) * 13, (args[2].y + 3183421) * 4 - 4201432, (args[2].z * 41 - 581357) * 2)
					end
		...
		elseif method == "FindPartOnRayWithIgnoreList" then -- for bullet redirection in the legitbotz
			...
			if passed then
				if Menu["Misc"]["Weapon Modifications"]["Enabled"]["Toggle"]["Enabled"] and Menu["Misc"]["Weapon Modifications"]["No Spread"]["Toggle"]["Enabled"] then
					args[1] = Ray.new(camera.CFrame.p, camera.CFrame.lookVector * legitbot.weaponstats.gunrange * 0.0694)
				end
...
do --ANCHOR fixes
	...
	local function HatObject(bool)
		for i,v in next, (getconnections(replicatedStorage.Events.HatObject.OnClientEvent)) do
			if bool then
				v:Disable()
			else
				v:Enable()
